...
const renderSummaryItems = (data) => {
    renderSummaryContainer.innerHTML = data.map((item) => {
        return `
        <section 
            class="p-2 w-full rounded-xl flex flex-row flex-wrap gap-[4%] items-center justify-center"
            style="background-color: ${item.bgcolorclass}; color: ${item.textcolorclass};"
            >
                <img class="flex-[0_0_10%] object-contain" src="${item.icon}"/>
                <p class="flex-[0_0_45%] overflow-hidden font-semibold">${item.category}</p>
